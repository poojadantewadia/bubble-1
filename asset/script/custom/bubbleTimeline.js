class BubbleTimelineChart {
    ...
    this.mappingObject = [
      {
        ...
        filedValue: 'Majority-minority seats',
      },
      {
        label: 'POC ELECTED FROM MM SEATS',
        filedValue: 'Elected from MM seats',
      },
      {
        label: 'POC ELECTED FROM NON-MM SEATS',
        filedValue: 'Elected non-MM seats',
      },
    ];
    this.myColor = d3
      .scaleOrdinal()
      .domain([
        ...
        'POC ELECTED FROM MM SEATS',
        'POC ELECTED FROM NON-MM SEATS',
      ])
  ...
  createEle() {
    ...
    this.headerTextG = parenG
      .append('g')
      .attr('class', 'headerTextG')
      .append('text')
      .attr('stroke', 'white')
  ...
  loadData() {
    ...
    d3.csv(this.dataPath).then((e) => {
      ...
      const getAllMajorityMinoritySeats = this.data.map((d) =>
        Number(d['Majority-minority seats'])
      );
      const getAllElectedFromMMSeats = this.data.map((d) =>
        Number(d['Elected from MM seats'])
      );
      const getAllElectedNonMMSeats = this.data.map((d) =>
        Number(d['Elected non-MM seats'])
      );
      ...
      this.cricleScale.domain([
        0,
        d3.max([
          ...getAllMajorityMinoritySeats,
          ...getAllElectedFromMMSeats,
          ...getAllElectedNonMMSeats,
        ]),
      ]);
  ...
  updatePath() {
    ...
    selectCircle
      ...
      .on('mouseenter', function (d) {
        ...
        const html = `<div style="color:black;font-size:18px; font-weight:bold;margin-bottom:1px;">${
          d.filedValue
        }</div>
        <div style="color:black;margin-bottom:1px;">${d3.timeFormat('%Y')(d.Start)}</div>
  ...
  getFilteData(d) {
    const dat = this.data.map((e) => {
      console.log(new Date(e.Start+'-1-1'),'getDate')
      return {
